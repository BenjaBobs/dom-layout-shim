...
markdown.renderer.rules.fence = (tokens, index) => {
  ...
  const metadata = Object.fromEntries(
    [...token.info.matchAll(/([a-z]+)="([^"]*)"/g)].map(match => [
      match[1],
      match[2],
    ]),
  );
  return renderCodeBox(token.content, {
    language,
    title: metadata.title,
    source: metadata.source,
    startLine: metadata.start,
    lineRanges: metadata.lines,
  });
};
...
export function renderCodeBox(
  ...
  const safeStartLine =
    Number.isSafeInteger(parsedStartLine) && parsedStartLine > 0
      ? parsedStartLine
      : 1;
  const ranges = lineRanges
    ? ` data-line-ranges="${markdown.utils.escapeHtml(lineRanges)}"`
    : '';
...
export function guideLayout({ attributes, markdown, tokens }) {
  ...
  if (firstSection < 0)
    throw new Error('Guide requires at least one level-two heading');
  ...
  const heroCode = heroTokens.find(
    token => token.type === 'fence' && token.info.includes('hero'),
  );
  if (!heading || !introduction || !heroCode)
    throw new Error(
      'Guide requires a title, introduction, and hero code fence',
    );
  ...
  const tableOfContents = sections
    .map(
      section =>
        `<a href="#${section.id}">${markdown.utils.escapeHtml(section.title)}</a>`,
    )
    .join('');
...
function splitSections(tokens) {
  ...
  for (let index = 0; index < tokens.length; ) {
    if (tokens[index].type !== 'heading_open' || tokens[index].tag !== 'h2') {
      throw new Error(
        'Guide content before the first section must belong to the hero',
      );
    }
  ...
  return (
    {
      ...
      json: '{}',
      text: 'TXT',
    }[language.toLowerCase()] || language.slice(0, 4).toUpperCase()
  );
}
