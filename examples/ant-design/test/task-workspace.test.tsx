...
describe('Ant Design task workspace consumer', () => {
  ...
  it('blocks an underlying control with the portalled modal and recomputes after deletion', async () => {
    ...
    const addTask = requiredElement<HTMLElement>(
      '[data-layout-key="add-task"]',
    );
    const firstMenu = requiredElement<HTMLElement>(
  ...
  it('filters, creates tasks, and dismisses deletion by clicking the mask', async () => {
    ...
    await enterText(
      requiredElement<HTMLInputElement>('#ant-task-title'),
      'Follow up with maintainers',
    );
    await enterText(
      requiredElement<HTMLTextAreaElement>('#ant-task-description'),
