...
describe('basic workspace consumer', () => {
  ...
  it('computes geometry and hit tests through the public package API', async () => {
    ...
    expect({ width: rect.width, height: rect.height }).toEqual({ width: 120, height: 40 })
    expect(document.elementFromPoint(rect.left + rect.width / 2, rect.top + rect.height / 2)).toBe(save)
  })
})
