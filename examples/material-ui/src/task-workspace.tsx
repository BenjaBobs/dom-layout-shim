...
const initialTasks: Task[] = [
  {
    ...
    description: 'Check the narrow viewport behavior and record any layout limitations.',
    status: 'Active',
  },
  {
    id: 2,
    title: 'Verify the release checklist',
    description: 'Confirm documentation, parity records, and package output are current.',
    status: 'Completed',
  },
  {
    id: 3,
    title: 'Prepare compatibility notes',
    description: 'Summarize overlay positioning and coordinate-based interaction results.',
    status: 'Active',
  },
]
  ...
  return (
    <Box sx={{ minHeight: '100vh', bgcolor: '#f4f6f8', color: '#17212b' }}>
      ...
      <Box sx={{ display: 'flex', width: '100%', maxWidth: 960, margin: '0 auto', padding: 3, gap: 3, boxSizing: 'border-box' }}>
        ...
        <Box component="main" sx={{ flex: 1, minWidth: 0 }}>
          <Stack direction="row" justifyContent="space-between" alignItems="center" sx={{ marginBottom: 2 }}>
            <Box>
              <Typography variant="h5" component="h2">Release preparation</Typography>
          ...
          <List aria-label="Project tasks" sx={{ height: 360, overflow: 'auto', bgcolor: '#fff', padding: 1, borderRadius: 2 }}>
            ...
            {visibleTasks.length === 0 && (
              <Typography sx={{ padding: 3, textAlign: 'center' }} color="text.secondary">No tasks match this filter</Typography>
            )}
          </List>
        </Box>
      </Box>
      ...
      >
        ...
        <DialogContent>
          <DialogContentText>
            {selectedTask ? `“${selectedTask.title}” will be permanently removed.` : 'This task will be permanently removed.'}
          </DialogContentText>
        </DialogContent>
        <DialogActions>
      ...
      <Snackbar open={noticeOpen} autoHideDuration={4000} onClose={() => setNoticeOpen(false)}>
