...
const generatedTypeScriptPath = resolve(
  root,
  'src/api/css-support-inventory.generated.ts',
);
const schemaSource = await readFile(
  resolve(root, 'support/css-support.schema.json'),
  'utf8',
);
...
if (errors.length > 0) {
  ...
  console.error(
    'Usage: node scripts/css-support.mjs <validate|generate|check|query> [query] [--json]',
  );
  process.exitCode = 1;
}
...
function validateRecords(records) {
  ...
  for (const record of records) {
    ...
    requireValue(
      errors,
      location,
      typeof record.id === 'string' && idPattern.test(record.id),
      'id must be kebab-case',
    );
    requireValue(
      errors,
      location,
      basename(location, '.json') === record.id,
    ...
    requireValue(
      ...
      `unknown effect "${record.effect}"`,
    );
    requireValue(
      errors,
      location,
      owners.has(record.owner),
      `unknown owner "${record.owner}"`,
    );
    requireValue(
      errors,
      location,
      Array.isArray(record.subjects?.properties),
      'subjects.properties must be an array',
    );
    requireValue(
      errors,
      location,
      Array.isArray(record.subjects?.elements),
      'subjects.elements must be an array',
    );
    requireValue(
      errors,
      location,
    ...
    for (const claim of record.claims ?? []) {
      ...
      requireValue(
        errors,
        claimLocation,
        typeof claim.id === 'string' && idPattern.test(claim.id),
        'claim id must be kebab-case',
      );
      requireValue(
        errors,
        claimLocation,
      ...
      requireValue(
        errors,
        claimLocation,
        supportValues.has(claim.support),
        `unknown support "${claim.support}"`,
      );
      requireValue(
        errors,
        claimLocation,
        parityValues.has(claim.parity?.status),
        `unknown parity status "${claim.parity?.status}"`,
      );
      requireValue(
        errors,
        claimLocation,
        Array.isArray(claim.parity?.fixtures),
        'parity.fixtures must be an array',
      );
      requireValue(
        errors,
        claimLocation,
      ...
      requireValue(
        errors,
        claimLocation,
        claim.properties === undefined || Array.isArray(claim.properties),
      ...
      if (claim.parity?.status === 'verified') {
        requireValue(
          errors,
          claimLocation,
          claim.parity.fixtures.length > 0,
          'verified claims require fixture evidence',
        );
      }
      if (claim.parity?.status === 'not-applicable') {
        requireValue(
          ...
          claimLocation,
          claim.parity.fixtures.length === 0,
          'not-applicable claims cannot reference fixtures',
        );
      }
      ...
      for (const note of claim.notes ?? []) {
        requireValue(
          errors,
          claimLocation,
          noteKinds.has(note.kind),
          `unknown note kind "${note.kind}"`,
        );
        requireValue(
          errors,
          claimLocation,
    ...
    const scopedClaims = (record.claims ?? []).filter(
      claim => claim.properties !== undefined,
    );
    ...
    if ((record.claims ?? []).some(claim => claim.support !== 'unsupported')) {
      for (const property of record.subjects?.properties ?? []) {
        ...
        requireValue(
          ...
          location,
          previous === undefined,
          `property "${property}" is also active in ${previous}`,
        );
...
async function generate(records) {
  ...
  console.log(
    `Generated ${records.length} records for the package and docs site.`,
  );
}
...
async function checkGenerated(records) {
  ...
  console.log(
    `Generated CSS support data is current (${records.length} records).`,
  );
}
...
function buildGeneratedOutputs(records) {
  ...
  const propertyIndex = Object.fromEntries(
    normalized
      .flatMap(record =>
        record.subjects.properties.map(property => [property, record.id]),
      )
...
function query(records, arguments_) {
  ...
  if (!queryText) {
    console.error(
      'Provide a property, value, feature, fixture, or note to search for.',
    );
    process.exitCode = 1;
    return;
  }
  ...
  const exactPropertyMatches = records.filter(record =>
    record.subjects.properties.includes(property),
  );
  const matches =
    exactPropertyMatches.length > 0
      ...
      : records.filter(record =>
          JSON.stringify(record).toLowerCase().includes(terms),
        );
  if (json) {
    console.log(
      JSON.stringify(
        matches.map(({ sourceFile, ...record }) => ({
          ...record,
          source: sourceFile,
        })),
        null,
  ...
  for (const record of matches) {
    ...
    console.log(
      `  properties: ${record.subjects.properties.join(', ') || 'none'}`,
    );
    console.log(
      `  support: ${aggregateSupport(record.claims)}; parity: ${aggregateParity(record.claims)}`,
    );
...
function aggregateParity(claims) {
  ...
  if (values.has('verified') || values.has('partially-verified'))
