...
export function compareCompatibilityRuns(
  ...
  const steps = chromium.checkpoints.map((expected, index) => {
    ...
    for (const selector of [
      ...new Set([
        ...Object.keys(expected.elements),
        ...Object.keys(actual.elements),
      ]),
    ]) {
      ...
      if (!browserElement || !engineElement) {
        discrepancies.push({
          selector,
          category: 'presence',
          expected: Boolean(browserElement),
          actual: Boolean(engineElement),
        });
        continue;
      }

      visibilityTotal += 1;
      if (browserElement.visible === engineElement.visible)
        visibilityMatches += 1;
      else
        discrepancies.push({
          selector,
          category: 'visibility',
          expected: browserElement.visible,
          actual: engineElement.visible,
        });

      for (const field of ['x', 'y', 'width', 'height']) {
        geometryTotal += 1;
        const difference = round(
          engineElement.rect[field] - browserElement.rect[field],
        );
        if (Math.abs(difference) <= 1) geometryMatches += 1;
          ...
          const relativeDifference =
            field === 'x' || field === 'y'
              ? round(
                  (engineElement.relativeRect ?? engineElement.rect)[field] -
                    (browserElement.relativeRect ?? browserElement.rect)[field],
                )
              : undefined;
          discrepancies.push({
            selector,
      ...
      for (const property of Object.keys(browserElement.layoutStyles ?? {})) {
        styleTotal += 1;
        const browserValue = canonicalLayoutStyleValue(
          property,
          browserElement.layoutStyles[property],
          browserElement.layoutStyles,
        );
        const engineValue = canonicalLayoutStyleValue(
          property,
          engineElement.layoutStyles?.[property] ?? '',
          engineElement.layoutStyles ?? {},
        );
        if (browserValue === engineValue) styleMatches += 1;
        else
          styleInputDifferences.push({
            selector,
      ...
      if (browserElement.centerHit === engineElement.centerHit) hitMatches += 1;
        ...
        discrepancies.push({
          ...
          expected: browserElement.centerHit,
          actual: engineElement.centerHit,
        });

      hitStackTotal += 1;
      const browserHitStack = browserElement.centerHitStack ?? [
        browserElement.centerHit,
      ];
      const engineHitStack = engineElement.centerHitStack ?? [
        engineElement.centerHit,
      ];
      if (JSON.stringify(browserHitStack) === JSON.stringify(engineHitStack))
        hitStackMatches += 1;
      else
        hitStackDifferences.push({
          selector,
          expected: browserHitStack,
          actual: engineHitStack,
        });
    }

    const coverageTotal = new Set([
      ...Object.keys(expected.elements),
      ...Object.keys(actual.elements),
    ]).size;
    const coverageCaptured = Object.keys(expected.elements).filter(
      selector => actual.elements[selector],
    ).length;
    const scores = {
    ...
    const discrepantSelectors = new Set(
      discrepancies.map(discrepancy => discrepancy.selector),
    );
    const relevantStyleInputDifferences = styleInputDifferences.filter(
      difference => discrepantSelectors.has(difference.selector),
    );
    const relevantHitStackDifferences = hitStackDifferences.filter(difference =>
      discrepantSelectors.has(difference.selector),
    );
  ...
  const unsupportedCss = summarizeUnsupportedCss(
    engine.unsupportedCss,
    supportInventory,
  );
  ...
  return {
    ...
    summary: {
      ...
      discrepancies: steps.reduce(
        (total, step) => total + step.discrepancies.length,
        0,
      ),
      uniqueDiscrepancies: discrepancyGroups.length,
      unstableCheckpoints: steps.filter(
        step =>
          !step.stability.chromium.stable || !step.stability.engine.stable,
      ).length,
      styleInputDifferences: steps.reduce(
        (total, step) => total + step.diagnostics.styleInputDifferences.length,
        0,
      ),
      hitStackDifferences: steps.reduce(
        (total, step) => total + step.diagnostics.hitStackDifferences.length,
        0,
      ),
    },
    steps,
...
function canonicalLayoutStyleValue(property, value, styles) {
  ...
  if (property === 'line-height') {
    ...
    if (unitless && fontSize)
      return `${round(Number(unitless[1]) * Number(fontSize[1]))}px`;
  ...
  return {
    async click({ selector }) {
      ...
      element.dispatchEvent(
        new document.defaultView.MouseEvent('mousedown', { bubbles: true }),
      );
      element.dispatchEvent(
...
export function captureDocument(document, step) {
  ...
  for (const selector of step.observe) {
    ...
    elements[selector] = {
      ...
      centerHitStack: Array.from(document.elementsFromPoint(center.x, center.y))
        .slice(0, 8)
        .map(identifyElement),
    };
  }
...
async function captureStableCheckpoint(driver, step, maximumAttempts = 5) {
  ...
  for (let attempt = 1; attempt <= maximumAttempts; attempt += 1) {
    ...
    if (key === previousKey)
      return { ...checkpoint, stability: { stable: true, attempts: attempt } };
    previousKey = key;
  }
  return {
    ...checkpoint,
    stability: { stable: false, attempts: maximumAttempts },
  };
}
...
function stabilityKey(checkpoint) {
  return JSON.stringify(
    Object.fromEntries(
      Object.entries(checkpoint.elements).map(([selector, element]) => [
        selector,
        {
          rect: element.rect,
  ...
  for (const step of steps) {
    for (const discrepancy of step.discrepancies) {
      const key = [
        ...
        discrepancy.field ?? '',
        JSON.stringify(discrepancy.expected),
        JSON.stringify(discrepancy.actual),
      ].join('\u0000');
...
function summarizeUnsupportedCss(
  summary = { unsupportedDeclarationCount: 0, declarations: [] },
  supportInventory = [],
) {
  ...
  return {
    ...
    properties: Array.from(properties.values())
      .map(property => ({
        ...
        priority: unsupportedCssPriority(
          property,
          supportByProperty.get(property.property) ?? [],
        ),
      }))
      .sort(
        (left, right) =>
...
function unsupportedCssPriority(property, support) {
  ...
  if (effects.has('visual-ignored') || effects.has('accepted-inert'))
    return 'visual-or-inert';
