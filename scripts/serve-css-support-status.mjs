...
if (
  ...
  server.listen(requestedPort, '127.0.0.1', () => {
    ...
    console.log(
      `CSS support overview: http://localhost:${port}/css-support-status.html`,
    );
  ...
  server.on('error', error => {
    if (error.code === 'EADDRINUSE') {
      console.error(
        `Port ${requestedPort} is already in use. Run with another port, for example:`,
      );
