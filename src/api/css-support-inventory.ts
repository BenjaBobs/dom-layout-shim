...
export function getInventoriedCssProperties(): string[] {
  return [
    ...new Set(
      cssSupportInventory.flatMap(record => record.subjects.properties),
    ),
  ].sort();
}
...
export function getInventoriedHtmlElements(): string[] {
  return [
    ...new Set(cssSupportInventory.flatMap(record => record.subjects.elements)),
  ].sort();
}
