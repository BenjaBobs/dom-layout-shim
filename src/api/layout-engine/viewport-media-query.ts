...
export function matchesViewportMediaQuery(
  ...
  return splitAtTopLevel(query, ',').some(part =>
    matchesSingleQuery(part.trim(), viewport),
  );
}
...
function matchesFeature(expression: string, viewport: Viewport): boolean {
  ...
  const orientation = feature.match(
  ...
  const aspectRatio = feature.match(
  ...
  const dimension = feature.match(
    /^(min-|max-)?(width|height)(?:\s*:\s*(.+))?$/i,
  );
  ...
  if (dimension) {
    ...
    return (
      expected !== undefined &&
      compare(actual, expected, dimension[1]?.toLowerCase())
    );
  }
...
function matchesRangeFeature(feature: string, viewport: Viewport): boolean {
  ...
  if (featureFirst) {
    ...
    return (
      expected !== undefined &&
      compareOperator(actual, expected, featureFirst[2] ?? '')
    );
  }

  const valueFirst = feature.match(/^(.+?)\s*(<=|>=|<|>|=)\s*(width|height)$/i);
  ...
  return (
    expected !== undefined &&
    compareOperator(expected, actual, valueFirst[2] ?? '')
  );
}
