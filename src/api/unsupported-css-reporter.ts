  ...
  return {
    onWarning(context) {
      ...
      if (existing) {
        ...
        if (context.element)
          existing.elements.add(describeElement(context.element));
      ...
      declarations.set(key, {
        ...
        elements: new Set(
          context.element ? [describeElement(context.element)] : [],
        ),
    ...
    getSummary() {
      const entries = Array.from(declarations.values())
        .map(
          (entry): UnsupportedCssSummaryEntry => ({
...
function readComputedValue(context: UnsupportedCssContext): string {
  if (
    !context.element ||
    context.property.startsWith('@') ||
    context.property === 'selector'
  )
    return '';
  return (
    context.element.ownerDocument.defaultView
      ?.getComputedStyle(context.element)
      .getPropertyValue(context.property) ?? ''
  );
}
