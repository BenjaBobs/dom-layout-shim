export function readDeclaration(declaration: unknown): {
  ...
  if (declaration.property === 'unparsed' && isRecord(declaration.value)) {
    ...
    if (
      isRecord(propertyId) &&
      typeof propertyId.property === 'string' &&
      Array.isArray(declaration.value.value)
    ) {
      return {
...
function stringifyTransformArgument(value: unknown): string {
  ...
  if (
    ['deg', 'grad', 'rad', 'turn'].includes(String(value.type)) &&
  ...
  if (
    ['a', 'b', 'c', 'd', 'e', 'f'].every(key => typeof value[key] === 'number')
  ) {
    return ['a', 'b', 'c', 'd', 'e', 'f']
...
function stringifyDisplay(value: Record<string, unknown>): string {
  ...
  if (value.type === 'pair') {
    ...
    if (
      ...
      if (
        value.outside === 'inline' &&
        (inside.type === 'flex' || inside.type === 'grid')
      ) {
...
function stringifyPlaceAlignment(
  ...
  const align = isRecord(value.align)
    ? stringifyPart(value.align)
    : JSON.stringify(value.align);
  const justify = isRecord(value.justify)
    ? stringifyPart(value.justify)
    : JSON.stringify(value.justify);
...
function stringifyFlex(value: Record<string, unknown>): string {
  const grow =
    typeof value.grow === 'number'
      ? String(value.grow)
      : JSON.stringify(value.grow);
  const shrink =
    typeof value.shrink === 'number'
      ? String(value.shrink)
      : JSON.stringify(value.shrink);
  const basis = isRecord(value.basis)
    ? stringifyLengthLike(value.basis)
    : JSON.stringify(value.basis);
...
function stringifyFlexFlow(value: Record<string, unknown>): string {
  const direction =
    typeof value.direction === 'string'
      ? value.direction
      : JSON.stringify(value.direction);
  const wrap =
    typeof value.wrap === 'string' ? value.wrap : JSON.stringify(value.wrap);
...
function stringifyGridTrackRepeat(value: Record<string, unknown>): string {
  const count =
    ...
    typeof value.count.value === 'number'
      ? value.count.value
      : undefined;
...
function stringifyRadiusPair(value: unknown): string {
  ...
  const firstValue = isRecord(first)
    ? stringifyRadiusLength(first)
    : JSON.stringify(first);
  const secondValue = isRecord(second)
    ? stringifyRadiusLength(second)
...
function stringifyTransformOriginPart(value: unknown): string {
  ...
  if (
    ...
    ['center', 'length', 'percentage'].includes(value.type)
  ) {
  ...
  if (value.type === 'side' && typeof value.side === 'string') {
    const offset = isRecord(value.offset)
      ? ` ${stringifyLengthLike(value.offset)}`
      : '';
...
function stringifyBackground(value: unknown): string {
  ...
  return isRecord(layer.color)
    ? stringifyColor(layer.color)
    : JSON.stringify(value);
}
  ...
  return value
    .map(layer => {
      ...
      if (layer.type === 'explicit') {
        const width = isRecord(layer.width)
          ? stringifyLengthLike(layer.width)
          : JSON.stringify(layer.width);
        const height = isRecord(layer.height)
          ? stringifyLengthLike(layer.height)
          : JSON.stringify(layer.height);
...
function stringifyInset(value: Record<string, unknown>): string {
  const sides = [value.top, value.right, value.bottom, value.left].map(
    stringifyInsetSide,
  );
...
function stringifyFontWeight(value: Record<string, unknown>): string {
  ...
  if (
    typeof value.type === 'string' &&
    ['normal', 'bold', 'bolder', 'lighter'].includes(value.type)
  ) {
    return value.type;
  }
  return JSON.stringify(value);
...
function stringifyVariable(variable: Record<string, unknown>): string {
  ...
  const name =
    isRecord(variable.value.name) &&
    typeof variable.value.name.ident === 'string'
...
function isDeclaration(
  value: unknown,
): value is { property: string; value: unknown } {
