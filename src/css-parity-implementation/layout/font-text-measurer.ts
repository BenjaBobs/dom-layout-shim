...
import type { TextMeasureInput, TextMeasurer, TextMeasureResult } from '../../api/text-measurer.ts'
...
export async function createDocumentFontTextMeasurer(
  ...
  const fetchFont = document.defaultView?.fetch?.bind(document.defaultView) ?? fetch
  ...
  for (const rule of readFontFaceRules(document, configuredStylesheets)) {
    for (const source of rule.sources) {
      ...
      try {
        ...
        faces.push({ family: normalizeFamily(rule.family), weight: rule.weight, font })
        break
      } catch {
...
function createFontTextMeasurer(faces: readonly LoadedFontFace[], fallback: TextMeasurer): TextMeasurer {
  return {
    measure(input) {
...
function selectFace(
  ...
  for (const family of splitFamilyList(familyList)) {
    const candidates = faces.filter((face) => face.family === normalizeFamily(family))
    if (candidates.length > 0) {
      return candidates.toSorted((a, b) =>
        Math.abs(a.weight - weight) - Math.abs(b.weight - weight))[0]
    }
  }
  return undefined
}
...
function measureWithFont(font: ParsedFont, input: TextMeasureInput): TextMeasureResult {
  const lines = breakLines(input, (text) => measuredWidth(font, text, input))
  return {
    width: lines.reduce((maximum, line) => Math.max(maximum, measuredWidth(font, line, input)), 0),
    height: lines.length * input.lineHeight,
  }
}

function measuredWidth(font: ParsedFont, text: string, input: TextMeasureInput): number {
  return font.getAdvanceWidth(text, input.fontSize, { kerning: true }) + text.length * (input.letterSpacing ?? 0)
}

function breakLines(input: TextMeasureInput, widthOf: (text: string) => number): string[] {
  const text = normalizeWhitespace(input.text, input.whiteSpace)
  if (!text) return []
  if (input.whiteSpace === 'pre' || input.whiteSpace === 'pre-wrap' || input.whiteSpace === 'pre-line') {
    return text.split('\n').flatMap((line) =>
      input.whiteSpace === 'pre-wrap' ? wrapLine(line, input.maxWidth, widthOf) : [line])
  }
  if (input.whiteSpace === 'nowrap' || !input.maxWidth || input.maxWidth <= 0) return [text]
  return wrapLine(text, input.maxWidth, widthOf)
}

function wrapLine(text: string, maxWidth: number | undefined, widthOf: (text: string) => number): string[] {
...
function normalizeWhitespace(text: string, whiteSpace: TextMeasureInput['whiteSpace']): string {
  const normalizedNewlines = text.replace(/\r\n?/g, '\n')
  if (whiteSpace === 'pre' || whiteSpace === 'pre-wrap') return normalizedNewlines
