...
type BenchmarkElement = NonNullable<ReturnType<BenchmarkDocument['querySelector']>>
...
await attachLayoutEngine({ window: primer, viewport: { width: 1024, height: 768 } })
primer.close()
...
async function runScenario(size: (typeof sizes)[number]): Promise<Scenario> {
  ...
  const subtreeMutationMs = await median(15, async () => {
    target.setAttribute('style', `width: ${mutationWidth++ % 2 === 0 ? 20 : 21}px`)
  ...
  const stylesheetInvalidationMs = await median(10, () => {
    stylesheet.textContent = stylesheetText(stylesheetWidth++ % 2 === 0 ? 10 : 11)
    target.getBoundingClientRect()
  })
...
async function benchmarkHotOperations(
...
): Promise<Pick<Scenario, 'cachedReadMs' | 'elementFromPointMs' | 'elementsFromPointMs'>> {
  ...
  bench
    ...
    .add('elementsFromPointMs', () => elementsFromPoint.elementsFromPoint(5, 5))
  await bench.run()
...
async function runInvalidations(target: BenchmarkElement, count: number): Promise<void> {
...
async function median(iterations: number, operation: () => unknown | Promise<unknown>): Promise<number> {
...
async function elapsed(operation: () => unknown | Promise<unknown>): Promise<number> {
...
function requiredElement(document: BenchmarkDocument, selector: string): BenchmarkElement {
...
function printResults(scenarios: readonly Scenario[], memoryGrowthBytes: number): void {
  ...
  console.log(`Heap growth after ${memoryInvalidations} invalidations: ${format(memoryGrowthBytes / 1024 / 1024)} MiB`)
}

function assertBudgets(scenarios: readonly Scenario[], memoryGrowthBytes: number): void {
  const failures: string[] = []
  for (const scenario of scenarios) {
    const scenarioBudgets = budgets[scenario.size as keyof typeof budgets]
    for (const name of Object.keys(scenarioBudgets) as (keyof ScenarioBudgets)[]) {
      if (scenario[name] > scenarioBudgets[name]) {
        failures.push(`${scenario.size} elements ${name}: ${format(scenario[name])} ms > ${scenarioBudgets[name]} ms`)
      }
    }
  }
  if (memoryGrowthBytes > memoryGrowthBudgetBytes) {
    failures.push(`memory growth: ${format(memoryGrowthBytes / 1024 / 1024)} MiB > 32 MiB`)
  }
