...
describe('affected scope classification', () => {
  ...
  it('validates classifier changes through unit and documentation checks', () => {
    expect(
      classifyAffectedScopes(['scripts/affected-scopes.mjs']),
    ).toMatchObject({
      package: true,
      parity: false,
      docs: true,
  ...
  it('runs package and parity checks for implementation changes', () => {
    expect(
      classifyAffectedScopes(['src/api/attach-layout-engine.ts']),
    ).toMatchObject({
      package: true,
      parity: true,
      docs: false,
  ...
  it('runs package tests for dependency issue synchronization changes', () => {
    expect(
      classifyAffectedScopes(['scripts/sync-dependency-update-issues.mjs']),
    ).toMatchObject({
      package: true,
      parity: false,
      docs: false,
  ...
  it('keeps deleted legacy documentation sources docs-only', () => {
    expect(
      classifyAffectedScopes(['docs-src/guide.template.html']),
    ).toMatchObject({
      package: false,
      parity: false,
      docs: true,
  ...
  it('validates package data and documentation for support inventory changes', () => {
    expect(
      classifyAffectedScopes(['support/css/flex-layout.json']),
    ).toMatchObject({
      package: true,
      parity: false,
      docs: true,
