...
describe('CSS support inventory', () => {
  it('tracks every declaration property handled by applyDeclaration', () => {
    ...
    const missing = handledDeclarationProperties().filter(
      property => !inventoried.has(property),
    );
  ...
  it('references existing browser parity fixtures', () => {
    const missing = cssSupportInventory.flatMap(entry =>
      entry.claims
        .flatMap(claim => claim.parity.fixtures)
        .filter(
          fixture =>
            !existsSync(
              resolve('test/browser-parity/cases', `${fixture}.test.ts`),
            ),
        )
  ...
  it('uses balanced inline-code markup in claim prose', () => {
    const malformed = cssSupportInventory.flatMap(entry =>
      entry.claims.flatMap(claim => [
  ...
  it('does not list the same implemented property in multiple active entries', () => {
    const activeProperties = cssSupportInventory
      ...
      .flatMap(entry =>
        entry.subjects.properties.map(property => ({
          entry: entry.id,
          property,
        })),
      )
    ...
    for (const item of activeProperties) {
      counts.set(item.property, [
        ...(counts.get(item.property) ?? []),
        item.entry,
      ]);
    }
...
function handledDeclarationProperties(): string[] {
  const source = readFileSync(
    resolve('src/css-parity-implementation/css/apply-declaration.ts'),
    'utf8',
  );
  ...
  return Array.from(
    propertySwitch.matchAll(/case '([^']+)'/g),
    match => match[1],
  ).sort();
}
