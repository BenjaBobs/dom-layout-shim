...
describe('CSS custom property resolution', () => {
  it('resolves nested references and fallbacks while preserving case-sensitive names', () => {
    ...
    expect(
      resolveCustomPropertyValue('calc(var(--nested) + 2px)', properties),
    ).toBe('calc( 40px + 2px)');
    expect(
      resolveCustomPropertyValue('var(--size)', properties),
    ).toBeUndefined();
  });
  ...
  it('invalidates cyclic properties but permits a fallback at the consuming declaration', () => {
    ...
    expect(resolveCustomPropertyValue('var(--a, 25px)', properties)).toBe(
      ' 25px',
    );
  });
  ...
  it('rejects malformed and non-variable functions', () => {
    ...
    expect(resolveCustomPropertyValue('myvar(--size)', properties)).toBe(
      'myvar(--size)',
    );
  });
});
