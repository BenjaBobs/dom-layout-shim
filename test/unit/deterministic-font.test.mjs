...
describe('deterministic parity font', () => {
  it('encodes the cross-platform metric contract in the generated binary', async () => {
    ...
    expect(font.getEnglishName('fontFamily')).toBe('DOM Layout Shim Deterministic')
    ...
    for (let codePoint = 0x20; codePoint <= 0x7e; codePoint += 1) {
      ...
      expect(font.hasChar(character), `missing ${JSON.stringify(character)}`).toBe(true)
      expect(font.charToGlyph(character).advanceWidth, JSON.stringify(character)).toBe(500)
    }
