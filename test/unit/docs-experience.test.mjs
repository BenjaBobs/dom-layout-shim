// biome-ignore-all lint/suspicious/noTemplateCurlyInString: these assertions match literal template source.
import { readFile } from 'node:fs/promises';
import { resolve } from 'node:path';
import { afterEach, describe, expect, it, vi } from 'vitest';

const readDocumentationSource = name =>
  readFile(resolve(process.cwd(), name), 'utf8');
...
describe('generated documentation experience', () => {
  it('renders accessible support controls and stable record links', async () => {
    const page = await readDocumentationSource(
      'docs-engine/css-support-status.template.html',
    );
    ...
    expect(page).toContain(
      'class="topic-permalink" href="#${escapeHtml(entry.id)}"',
    );
    expect(page).toContain(
      "{ label: 'Support topics', value: inventory.length",
    );
    expect(page).toContain(
      "class=\"metric${metric.status ? ' metric-filter' : ''}\"",
    );
    ...
    expect(page).toContain(
      'id="fixtureDialog" aria-labelledby="fixtureDialogTitle"',
    );
    ...
    const generator = await readDocumentationSource(
    ...
    expect(generator).toMatch(
      /\.filter\(\s*file => file\.endsWith\('\.test\.ts'\)\s*\)/,
    );
    expect(generator).toContain(
    ...
    expect(generator).toContain(
      'compatibility-report.json" target="_blank" rel="noopener"',
    );
    ...
    expect(generator).toMatch(
      /\.split\('\+'\)\s*\.map\(name => extractDocumentationRegion\(file, name, relativePath\)\)/,
    );
  ...
  it('renders support dimensions, semantic notes, inline code, and summary filters', async () => {
      ...
      claims: [
        {
          ...
          notes: [
            {
              kind: 'limitation',
              text: 'Resolution of `var()` is not implemented.',
            },
          ],
        },
      ],
    ...
    const fetch = vi.fn(async input =>
      String(input).includes('css-support.json')
        ? new Response(JSON.stringify({ records: [record] }))
        : new Response("it('matches Chromium', () => {})"),
    );
    vi.stubGlobal('fetch', fetch);

    // biome-ignore lint/security/noGlobalEval: the test executes the generated browser script inside happy-dom.
    window.eval(script);
    await vi.waitFor(() =>
      expect(document.querySelector('#rows').textContent).toContain(
        'CSS variables',
      ),
    );

    expect(document.querySelector('.status-field').textContent).toContain(
      'Implementation support',
    );
    expect(document.querySelector('.parity-verified').textContent).toContain(
      'Supported scope verified',
    );
    expect(
      document.querySelector('.note-limitation .note-kind').textContent,
    ).toBe('Known limitation');
    expect(document.querySelector('.note-text code').textContent).toBe('var()');
    expect(document.querySelector('.claim-description code').textContent).toBe(
      'var()',
    );
    expect(document.querySelector('.fixture-preview').textContent).toContain(
      'variables.test.ts',
    );
    expect(document.querySelector('.fixture-preview svg')).toBeNull();
    expect(
      document.querySelector('.claim-evidence .claim-label svg'),
    ).not.toBeNull();
    expect(
      document.querySelector('.claim').lastElementChild.classList,
    ).toContain('claim-evidence');
    expect(document.querySelector('.title > span').textContent).toBe(
      'CSS variables',
    );
    ...
    await vi.waitFor(() =>
      expect(
        document.querySelector('#fixtureDialogSource code').dataset.source,
      ).toContain('matches Chromium'),
    );
    expect(document.querySelector('#fixtureDialog').hasAttribute('open')).toBe(
      true,
    );
    expect(document.querySelector('#fixtureDialogGithub').href).toContain(
      '/variables.test.ts',
    );
    expect(fetch).toHaveBeenCalledWith(
      './data/parity-sources/variables.test.ts',
    );
  });
  ...
  it('renders anchored, filterable changelog releases with sticky context', async () => {
    ...
    expect(page).toContain(
      'id="${releaseId(release)}" data-release="${escapeHtml(release)}"',
    );
    expect(page).toContain("if (release === 'Upcoming') return 'upcoming'");
    expect(page).toMatch(/return `version-\$\{release\s*\.toLowerCase\(\)/);
  });
});
