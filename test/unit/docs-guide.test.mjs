...
describe('Markdown guide rendering', () => {
  it('renders authored Markdown fragments for embedded documentation', () => {
    expect(renderMarkdownFragment('Use `attachLayoutEngine`.')).toContain(
      '<code>attachLayoutEngine</code>',
    );
  });

  it('renders fenced source as a reusable linked code box', () => {
    const rendered = renderMarkdownFragment(
      '```ts title="vitest.config.ts" source="https://example.test/vitest.config.ts" start="12"\nconst value = true\n```',
    );

    expect(rendered).toContain('<figure class="code-box">');
    expect(rendered).toContain(
      '<span class="code-language" aria-hidden="true">TS</span>',
    );
    expect(rendered).toContain(
      '<a href="https://example.test/vitest.config.ts">vitest.config.ts</a>',
    );
    expect(rendered).toContain(
      '<button class="code-copy" type="button" aria-label="Copy code">',
    );
    expect(rendered).toContain(
      '<code data-language="ts" data-start-line="12">const value = true',
    );
  });
  ...
  it('renders standalone authored documentation pages', () => {
    ...
    expect(rendered.pageStyles).not.toContain(
      '.article a[href^="./examples/"]',
    );
  });
  ...
  it('renders metadata, the hero, navigation, and numbered sections', () => {
    ...
    expect(rendered.body).toContain(
      '<section id="first-step"><p class="eyebrow">Step 1</p>',
    );
    expect(rendered.body).toContain(
      '<section id="first-step-2"><p class="eyebrow">Step 2</p>',
    );
  ...
  it('labels code and escapes source HTML', () => {
    ...
    expect(rendered.body).toContain(
      '<code data-language="ts" data-start-line="1">',
    );
  ...
  it('requires guide metadata and structure', () => {
    expect(() => renderGuide('# Missing frontmatter')).toThrow(
      'Guide requires frontmatter',
    );
    expect(() =>
      renderGuide(`---
title: Test guide
