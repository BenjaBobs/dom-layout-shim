...
import {
  ...
  highlight,
  navigate,
} from '../../docs-engine/assets/site.js';
...
describe('documentation site behavior', () => {
  it('opens and closes the generated mobile navigation', () => {
    document.body.innerHTML =
      '<nav data-site-nav><button class="site-menu-button" aria-expanded="false"></button><div class="site-nav-links"><a href="./">Guide</a></div></nav>';

    enhanceNavigation();
    document.querySelector('button').click();
    expect(document.querySelector('nav').hasAttribute('data-menu-open')).toBe(
      true,
    );
  ...
  it('depth-colors brackets without treating brackets in strings as pairs', () => {
    const output = highlight(
      "const value = { nested: call([1]), label: '[' }",
      'ts',
    );
  ...
  it('adds line-numbered highlighted rows and copy behavior to code boxes', async () => {
    ...
    document.body.innerHTML =
      '<figure class="code-box"><figcaption><button class="code-copy"><span>Copy</span></button></figcaption><pre><code data-language="ts" data-start-line="8">const one = 1\nconst two = 2\n</code></pre></figure>';
    ...
    expect(document.querySelector('.code-line').innerHTML).toContain(
      'tok-keyword',
    );
    expect(document.querySelectorAll('.line-source')).toHaveLength(2);
    expect(
      [...document.querySelectorAll('.code-line')].map(
    ...
    await vi.waitFor(() =>
      expect(document.querySelector('.code-copy span').textContent).toBe(
        'Copied',
      ),
    );
  });
  ...
  it('preserves disjoint source ranges in one code box', () => {
    document.body.innerHTML =
      '<figure class="code-box"><pre><code data-language="ts" data-start-line="2" data-line-ranges="2-2,89-90">import value\n__DOCS_CODE_SKIP__\ncall({\n})</code></pre></figure>';
    ...
    expect(
      document.querySelector('.code-skip').getAttribute('aria-label'),
    ).toBe('Source lines omitted');
  });

  it('tracks the current guide section in the table of contents', () => {
    document.body.innerHTML =
      '<aside data-scrollspy><a href="#one">One</a><a href="#two">Two</a></aside><section id="one"></section><section id="two"></section>';
  ...
  it('opens external links in a separate tab without exposing the opener', () => {
    document.body.innerHTML =
      '<a href="./guide">Internal</a><a href="https://example.com">External</a>';

    enhanceExternalLinks(document.body);

    expect(document.querySelector('[href="./guide"]').target).toBe('');
    expect(document.querySelector('[href="https://example.com"]').target).toBe(
      '_blank',
    );
    expect(document.querySelector('[href="https://example.com"]').rel).toBe(
      'noopener noreferrer',
    );
  });
  ...
  it('navigates by replacing page content, styles, title, and history', async () => {
    ...
    expect(document.querySelector('[data-page-styles]').textContent).toContain(
      '.new',
    );
    expect(location.pathname).toBe('/changelog.html');
    expect(
      document
        .querySelector('[href="./changelog.html"]')
