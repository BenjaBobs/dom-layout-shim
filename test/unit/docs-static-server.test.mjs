...
beforeAll(async () => {
  await mkdir(resolve(fixtureRoot, 'examples/material-ui'), { recursive: true })
  await writeFile(resolve(fixtureRoot, 'index.html'), 'Docs')
  await writeFile(resolve(fixtureRoot, 'examples/material-ui/index.html'), 'Material UI')
})
...
describe('documentation static server routing', () => {
  it('resolves directory URLs to their index document', () => {
    expect(resolveDocumentationRequest(fixtureRoot, '/examples/material-ui/')).toEqual({
      status: 200,
  ...
  it('keeps resolved paths inside the documentation root', () => {
    expect(resolveDocumentationRequest(fixtureRoot, '/../package.json')).toEqual({
      status: 403,
      message: 'Forbidden',
    })
  })
