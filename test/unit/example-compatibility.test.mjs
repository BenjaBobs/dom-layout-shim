  ...
  it('treats equivalent computed typography values as matching diagnostics', () => {
    const chromium = { checkpoints: [{
      ...
      elements: { '#text': {
        ...
        visible: true,
        centerHit: '#text',
        layoutStyles: { 'font-size': '14px', 'font-weight': '400', 'line-height': '22px', 'white-space': 'normal' },
      } },
    }] }
    const engine = { checkpoints: [{
      id: 'initial',
      label: 'Initial',
      elements: { '#text': {
        ...
        visible: true,
        centerHit: '#text',
        layoutStyles: { 'font-size': '14px', 'font-weight': 'normal', 'line-height': '1.571429', 'white-space': '' },
      } },
    }] }

    const report = compareCompatibilityRuns('example', { id: 'scenario', steps: [] }, chromium, engine)
