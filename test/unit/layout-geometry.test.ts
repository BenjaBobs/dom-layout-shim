...
describe('layout DOM API and package contracts', () => {
  ...
  it('disables only presentation defaults with the none profile', async () => {
    document.body.innerHTML =
      '<p id="paragraph">Text</p><input id="hidden" type="hidden" style="display:block">';
  ...
  it('cascades user-agent overrides below document and inline styles', async () => {
    ...
    expect(
      requiredElement('#document-rule').getBoundingClientRect().y,
    ).toBeCloseTo(31.2);
    expect(
      requiredElement('#inline-rule').getBoundingClientRect().y,
    ).toBeCloseTo(59.4);
  });
  ...
  it('merges native-control overrides with the selected profile', async () => {
    ...
    expect({ width: textRect.width, height: textRect.height }).toEqual({
      width: 240,
      height: 23,
    });
    expect({ width: checkboxRect.width, height: checkboxRect.height }).toEqual({
      width: 18,
      height: 16,
    });
  });
  ...
  it('debug output identifies the element blocking another element at its center', async () => {
    ...
    expect(debugLayout(window)).toContain(
      'button#save x=10 y=10 w=100 h=40 z=0 pe=auto visibility=visible BLOCKED_BY=div#overlay',
    );
  });
  ...
  it('asserts pointer reachability with debug output on failure', async () => {
    ...
    expect(() =>
      expectReceivesPointer(requiredElement('#overlay')),
    ).not.toThrow();
    expect(() => expectReceivesPointer(requiredElement('#save'))).toThrow(
      /Top element: div#overlay/,
    );
    expect(() => expectReceivesPointer(requiredElement('#save'))).toThrow(
      /Layout debug:/,
    );
  ...
  it('asserts center blockers and returns the blocking element', async () => {
    ...
    expect(expectBlockedBy(requiredElement('#save'))).toBe(
      requiredElement('#overlay'),
    );
    expect(() =>
      expectBlockedBy(requiredElement('#save'), requiredElement('#overlay')),
    ).not.toThrow();
    expect(() => expectBlockedBy(requiredElement('#overlay'))).toThrow(
      /Expected div#overlay to be blocked/,
    );
  });
  ...
  it('guards click dispatch with pointer reachability', async () => {
    ...
    expect(() => guardedClick(requiredElement('#save') as HTMLElement)).toThrow(
      /Top element: div#overlay/,
    );
